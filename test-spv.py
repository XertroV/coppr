...
ETH.accounts['alice'] = 10**25
	
testTransactions(ETH, transactions_chainheaders)
testTransactions(ETH, transactions_merkletracker)
spv_results = testTransactions(ETH, transactions_spv)

testResults(spv_results)

#ETH.contracts['CHAINHEADERS'].printState()
#ETH.contracts['MERKLETRACKER'].printState()
ETH.contracts['SPV'].printState()
